  ...
  async function navigate(url, { push = true } = {}) {
    ...
    const swapBody = async () => {
      ...
      Array.from(doc.body.childNodes).forEach((node) => {
        // Skip sidebar-root if preserving sidebar to avoid flash
        if (preserveSidebar && node.id === 'sidebar-root') return;
        // Skip glow-layer to prevent flash during navigation
        if (node.classList && node.classList.contains('glow-layer')) return;
      ...
      // Yield to the event loop so Tailwind's MutationObserver can compile newly 
      // injected utility classes BEFORE the View Transition takes the "new" snapshot.
      // This is critical - if Tailwind hasn't compiled, the snapshot captures unstyled content.
