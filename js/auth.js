...
function kirRenderSidebarNow(activeTab) {
  ...
  const adminRoles = ['Ketua Ekstrakurikuler', 'Wakil Ketua', 'Bendahara'];
  const isAdmin = role && adminRoles.includes(role);
  const adminHiddenClass = isAdmin ? '' : 'hidden';
  ...
  <aside id="sidebar" class="hidden lg:flex lg:flex-col w-full lg:h-screen glass border-y-0 border-l-0 px-4 py-6 lg:sticky lg:top-0 relative ${kirSidebarCollapsedClass()}">
    ...
    <nav class="flex flex-col gap-1.5">
      ...
      <a href="admin.html" data-tab="admin" data-admin-only="true" class="nav-link ${activeTab === 'admin' ? 'active' : ''} flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-zinc-300 ${adminHiddenClass}">
    ...
    <nav class="flex flex-col gap-1.5 mt-6">
      ...
      <a href="inbox.html" data-tab="inbox" data-admin-only="true" class="nav-link ${activeTab === 'inbox' ? 'active' : ''} flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-zinc-300 ${adminHiddenClass}">
        <span class="relative shrink-0 inline-flex">
